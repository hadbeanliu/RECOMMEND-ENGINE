...
//	    		options.search($(this));
	    		options.search($(this, $(".v_oper_click_search_input").get()));
	    	}
	    })
...
//	    	options.search($qsearch);
	    ...
	    var $csearch = $oper.find('.v_oper_click_search').click(function(){
	    	var $elem = $(this);
	    	var $inputSearch = $elem.siblings("input[name="+this.name+"]");
...
//    			options.afterSearch();
	    		...
	    		if ($elem.parent().hasClass('v_oper_click_search_enable_remove_active')) {
	    			...
	    			$elem.siblings().remove(".v_oper_click_search_input");
	    		}
	    		
	    	} else {
	    		$elem.siblings().remove(".v_oper_click_search_input");
	    		if (this.name) {
	    			$inputSearch = $("<input class=\"v_oper_click_search_input\" name="+this.name+" type=\"hidden\" />");
	    			$inputSearch.val($elem.attr('value') || '');
	    			$elem.parent().append($inputSearch);
...
//	    			options.afterSearch();
	    ...
	    $csearch.filter(".active").each(function(){ // 初始化操作
	    	var $elem = $(this);
	    	$elem.siblings().remove(".v_oper_click_search_input");
	    	var $inputSearch = $("<input class=\"v_oper_click_search_input\" name="+this.name+" type=\"hidden\" />");
			$inputSearch.val($elem.attr('value') || '');
			$elem.parent().append($inputSearch);
	    });
...
//				options.def = defaultOptions; /* 提供默认操作 */
				...
				if ( ! ulLiMode && options.columns && options.columns.length > 0 ) {
					tableHead.push('<table class="display hide '+ tableSelector +'"><thead><tr>');
...
//					tableSelector = '.' + tableSelector;
				...
				if ( ! options.initSearch ) {
					var ls = dtPageUtil.getFormDataArray($page);
					if (ls && ls.length > 0) {
						options.initSearch = ls.join("&");
					}
				}
...
/*			    	console.log(processing ? '加载中。。。' : '加载完成'); */
			    	...
			    	if ( ! processing ) {
			    		...
			    		if (isFirst) {
			    			$table.find('table').removeClass('hide');
			    			isFirst = false;
			    		}
			    		$table.find('.dataTables_paginate .unavailable a').click(function(){
			    			return false;
			    		}).end().find('.icon-more').hover(function(){
