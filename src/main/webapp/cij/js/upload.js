(function( $ ){
    ...
    var editMore = {};
    editMore.editForm = function($form, liid, call){
    	if (typeof call === 'function') {
			call(); // 自定义值设置
		} else {
			clearPhotoEditInfo(); //清空编辑器数据
			editMore.fillFormValue($form, liid);
		}
    }
    editMore.fillFormValue = function ($form, liid) { /* 填充u到$form中 */
    	var num = getArrIndex(liid);
    	$form.find('#right-label').val(baseMode.arr_images[num].uploadTime);
    	$form.find('#tpmc').val(baseMode.arr_images[num].name);
    	$form.find('#tpms').val(baseMode.arr_images[num].description);
    	$form.find('#psdd').val(baseMode.arr_images[num].address);
    	$form.find('#saveimg').attr("uid", liid).attr("data-num", num);
		var saen = $form.find('.v_oper_submit_and_edit_next');
		saen.attr("data-num",num);
		$form.find('.v_oper_submit').attr("data-num",num);
    } 
	...
	xiuxiu.onInit = function (id)
	{
        ...
        xiuxiu.setUploadURL("http://127.0.0.1/fupload/fileUpload.sp?act=uploadSave&appId=00000001&caseId=00000019");
    	...
    	batchManageBtn:function(){
			...
			$('#editBtn').click(function() {
				...
				if (num == "0") {
					...
					var $form = $('#edit_imginfo');
					var lis = [];
					$checkImg.each(function() {
						lis.push($(this).parents('li'));
					});
					page.edit(lis,$form);
				}
				//清空编辑框
//				clearPhotoEditInfo();
	    	...
	    	$("#v_oper_submit").click(function() {
	    		...
	    		var  FileIdSort = sort.toArray(); //id排序
				var tem_images = baseMode.arr_images; //临时空间
				baseMode.arr_images = []; //清空 arr_images
				for (i = 0; i < FileIdSort.length; i++) {
					for (k = 0; k < tem_images.length; k++) {
						if (tem_images[k].wuFileId == FileIdSort[i]) {
							//默认第一张
//							if(i==0){
//								tem_images[k].isCover=1;
//							}else{
//								tem_images[k].isCover=0;
//							}
							baseMode.arr_images.push(tem_images[k]);
							break;
						}
					}
				}
				//附件图片的json类型
				var jsonimages = JSON.stringify(baseMode.arr_images);
				var vo=RJ.E.util.getFormData($form);
				vo["imagesFile"]=jsonimages;
				var $panel=$('.v_info_warning');
				$.ajax({
					type : "POST",
					async: false,//改为同步方式
					url : "/emall/seller/goods/save",
					data : vo,
					dataType : "json",
					success : function(res) {
						$panel.addClass("show");
				    	$panel.find(".v_show_info").text(res.Msg);
			    		setTimeout(function() {
					...
					error : function(e) {
						$panel.addClass("show");
				    	$panel.find(".v_show_info").text("发布失败："+e);
			    		setTimeout(function() {
			    			$panel.removeClass("show");
			    		}, 3000);
					}
		...
		//保存照片信息
		saveInfoBtn: function() {
			var page = this;
			$('#saveimg').click(function() {
				//单张编辑
				var num = $(this).attr("data-num");
				baseMode.arr_images[num].uploadTime = $('#right-label').val();
				baseMode.arr_images[num].name = $('#tpmc').val();
				baseMode.arr_images[num].description = $('#tpms').val();
				baseMode.arr_images[num].address = $('#psdd').val();
				jqBaseapp.popup("保存成功", "1");
				$('#edit_imginfo').foundation('reveal', 'close');
			});
			$('#saveimg1').click(function() {
				//批量编辑
				var num = $('#check-num').html();
				if (num == "0") {
					jqBaseapp.popup("请选择文件", "1");
				} else {
					$('#uploader').find('.filelist .checked').each(function() {
						if ($('#tpmc1').val() == "") {
							jqBaseapp.popup("图片名称不能为空", "2");
							return;
						}
						var fileId = $(this).parents('li').attr("id");
						var num = getArrIndex(fileId);
						baseMode.arr_images[num].uploadTime = $('#right-label1').val();
						baseMode.arr_images[num].name = $('#tpmc1').val();
						baseMode.arr_images[num].description = $('#tpms1').val();
						baseMode.arr_images[num].address = $('#psdd1').val();
		...
		edit: function(lis,$form){
    		var self = this;
    		var saen = $form.find('.v_oper_submit_and_edit_next');
    		saen.css('display', lis.length > 1 ? 'block' : 'none');
    		$form.find('.v_oper_submit').css('display', lis.length > 1 ? 'none' : 'block');
    		if (lis.length > 1) {
    			saen.unbind('click');
    			saen.click(function(){
    				self.save($form, function(){
	    				if (lis.length > 0) {
	    					if (lis.length == 1) {
	    						$form.find('.v_oper_submit').css('display', 'block');
	    						saen.css('display', 'none');
	    					}
	    					self.editForm($form, lis.shift().attr("id"));//gaigaigiagiaigiagiaigaigiaiigaigiaigiagi
	    				}
    				});
    			});
    		}
    		var first = lis.shift();
    		this.editForm($form, first.attr("id"));
...
//    			saen.unbind('click');
//    			saen.click(function(){
//    				self.save(num,function(){
//	    				self.edit(saen.attr("data-num")-1);
//    				});
//    			});
//    		}
	    },editForm: function($form,liid){
	    	var self = this;
	    	editMore.editForm($form, liid, function(){
	    		self.editFormById($form, liid);
	    	});
	    },
	    editFormById: function($form, liid){
	    	var num = getArrIndex(liid);
	    	$form.find('#right-label').val(baseMode.arr_images[num].uploadTime);
	    	$form.find('#tpmc').val(baseMode.arr_images[num].name);
	    	$form.find('#tpms').val(baseMode.arr_images[num].description);
			...
			$form.find('.v_oper_submit').attr("data-num",num);
	    	this.afterEdit($form);
	    },afterEdit: function($form){
	    	if ( ! $form.hasClass("open") ) {
	    		$form.foundation("reveal", "open");
	    	}
	    },
	    save: function($form,call){
	    	this.doSave($form,call);
	    },
	    doSave: function($form,call){
	    	var saen = $form.find('.v_oper_submit_and_edit_next');
			var num = saen.attr("data-num");
			if(!num){
				num = $form.find('.v_oper_submit').attr("data-num");
			}
			if(!num){
				num = $form.find('#saveimg').attr("data-num");
			}
	    	baseMode.arr_images[num].uploadTime = $('#right-label').val();
			baseMode.arr_images[num].name = $('#tpmc').val();
			baseMode.arr_images[num].description = $('#tpms').val();
    ...
    var init =  {
        	editGooods : function(){
        		...
        		$.ajax({
    				type: "GET",
    				dataType: 'json',
    				url: Cfg.contextPath + "/seller/goods/goodsImages",
    				data: {
    					"arId": arId
    				},
...
//    								address: "gdfg"
    								...
    								make: "",
    								model: "",
    								name: im.rsName+"."+im.rsSuffix,
    								raw: im.rsUrl,
    								shutterSpeedValue: "",
    								thumbnail_url: im.thumbnailUrl,
    								type: im.fileType,
    								uploadTime: new Date(im.uploadTime).format("yyyy-MM-dd hh:mm:ss"),
    								url: im.sourceUrl,
									isCover: im.isCover?im.isCover:"0",
									fileSuffix: im.rsSuffix,
									rsId: im.rsId
    						};
    						if(im.isCover=="1"){
    							thumData[i]=thumData[0];
        						thumData[0]=image;
    						}else{
        						thumData[i]=image;
    						}
	...
	var getArrIndex= function(wuFileId) {
        ...
        uploader = WebUploader.create({
            ...
            chunked: false,
            chunkSize: 512 * 1024,
            server:'/fupload/fileUpload.sp?act=uploadSave&appId=00000001&caseId=00000019',
       ...
       uploader.on('filesQueued', function() {
		...
		function addEditFile(file, i) {
			...
			var $li = $('<li id="' + fileId + '" data-type="1" data-id="' + fileId + '" class="state-complete">' +
				'<div class="covers '+(isCover=='1'?"active":"")+'" title="点击设置封面">封面</div>'+
			...
			var $btns = $('<div class="file-panel">' +
					...
					'<div class="moreBtn"><a title="更多"><i class="fa fa-ellipsis-h"></i></a>'+
                ...
                img.setAttribute('src', file.raw);
//				img = $('<img src="' + file.raw + '">');
				$wrap.empty().append(img);
				img.onload = function (){
                	scale(this, this.parentNode.offsetWidth, this.parentNode.offsetHeight);
                };
				var isHide = $("#manageBar").is(":hidden"),
				...
				$btns.on('click','i',function(){
					var index = $(this).index();
					 switch ( index ) {
	                    case 0:
	                		var saen = $('.v_oper_submit_and_edit_next');
	                		saen.css('display', 'none');
	                    	$('#edit_imginfo').foundation('reveal', 'open');
							clearPhotoEditInfo(); //清空编辑器数据
							$li = $(this).parents('li');
		...
		$(document).on("click",".covers",function(){
			...
			if($isCover){
				var wuFileId = $(this).parents('li').attr("id");
				var num = getArrIndex(wuFileId);
				baseMode.arr_images[num].isCover="0";
			}
			var $covers = $('.covers');
			var wuFileId = $(this).parents('li').attr("id");
			var num = getArrIndex(wuFileId);
			if($(this).hasClass('active')){
				...
				baseMode.arr_images[num].isCover="1";
				$covers.removeClass('active');
				$(this).toggleClass('active');
				$(this).attr("id","isCover");
			}
		});
        // 负责view的销毁
