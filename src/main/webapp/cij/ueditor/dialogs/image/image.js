...
(function () {
    ...
    function setTabFocus(id) {
        ...
        switch (id) {
            ...
            case 'relate':
                setAlign(editor.getOpt('imageManagerInsertAlign'));
                relateImage = relateImage || new RelateImage('imageRelateList');
                relateImage.reset();
                break;
            case 'search':
                setAlign(editor.getOpt('imageManagerInsertAlign'));
        ...
        dialog.onok = function () {
            ...
            switch (id) {
                ...
                case 'online':
                    ...
                    break;
                case 'relate':
                    list = relateImage.getInsertList();
                    break;
                case 'search':
    ...
    OnlineImage.prototype = {
        ...
        initData: function () {
            ...
            /* 拉取数据需要使用的值 */
            this.state = 0;
            this.listSize = editor.getOpt('imageMineListSize');
            this.listIndex = 1;
            this.total = 1;
            this.listEnd = false;
        ...
        getImageData: function () {
            ...
            if(!_this.listEnd && !this.isLoadingData) {
                ...
                var url = editor.getActionUrl(editor.getOpt('imageMineActionName')),
        ...
        pushData: function (list) {
            var i, item, img, icon, _this = this,
                urlPrefix = editor.getOpt('imageMineUrlPrefix');
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].sourceUrl) {
                    ...
                    img.width = 113;
                    img.setAttribute('src', list[i].sourceUrl);
                    img.setAttribute('_src', list[i].sourceUrl);
    ...
    function RelateImage(target) {
        this.container = utils.isString(target) ? document.getElementById(target) : target;
        this.init();
    }
    RelateImage.prototype = {
        init: function () {
            this.reset();
            this.initEvents();
        },
        ...
        getImageData: function () {
            ...
            if(!_this.listEnd && !this.isLoadingData) {
                ...
                ajax.request(url, {
                    ...
                    'method': 'get',
                    'onsuccess': function (r) {
                        var data = isJsonp ? r:eval('(' + r.responseText + ')');
                    	var thumData = [];
    					for(var i in data){
    						var im=data[i];
    						var image = {
    					...
    					_this.pushData(thumData);
    					_this.listIndex = parseInt('0') + parseInt(thumData.length);
                        if(_this.listIndex >= thumData.length) {
                            _this.listEnd = true;
                        }
                        _this.isLoadingData = false;
//                        try {
